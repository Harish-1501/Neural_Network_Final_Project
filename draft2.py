...
# If your training used label '1' for the NEGATIVE/TOXIC class, invert.
invert_scores = st.sidebar.checkbox(
    "Invert probability (1 − p)",
    value=False,
    help=(
        "Enable if training used label '1' for the negative/toxic class. "
        "When on, we interpret output as p('" + pos_label + "') = 1 − raw_sigmoid."
    ),
)
    ...
    if submitted:
        if not text.strip():
            ...
            with st.spinner("Tokenizing and running the model…"):
                ...
                p_raw = float(model.predict(X, verbose=0)[0][0])
p = 1.0 - p_raw if invert_scores else p_raw
label = pos_label if p >= thresh else neg_label

st.markdown(f"**Prediction:** `{label}`  <span class='chip'>{p:.4f}</span>", unsafe_allow_html=True)
st.progress(p, text=f"Probability of '{pos_label}': {p:.2%}")
with st.expander("Debug", expanded=False):
    st.caption(f"raw sigmoid (model): {p_raw:.4f} · used (after invert toggle): {p:.4f}")
        ...
        if not text_cols:
            ...
            if run_batch:
                with st.spinner("Running predictions…"):
                    ...
                    probs_raw = model.predict(X, verbose=0).reshape(-1)
probs = (1.0 - probs_raw) if invert_scores else probs_raw
...
If predictions seem reversed, toggle <em>Invert probability</em> in the sidebar (common when label '1' means negative/toxic).  
